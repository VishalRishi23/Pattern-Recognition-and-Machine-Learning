...
def load_dir(dir, n, delim = '', nc = 2):
    data = []
    for file in os.listdir(dir):
        if delim in file:
            arr = np.loadtxt(dir + file)
            nf = arr[0]
            arr = np.reshape(arr[1:], (int(nf), nc))
...
coast_train_dir = 'Hand writing data/a/train/'
forest_train_dir = 'Hand writing data/ai/train/'
highway_train_dir = 'Hand writing data/ba/train/'
mountain_train_dir = 'Hand writing data/da/train/'
opencountry_train_dir = 'Hand writing data/la/train/'

coast_dev_dir = 'Hand writing data/a/dev/'
forest_dev_dir = 'Hand writing data/ai/dev/'
highway_dev_dir = 'Hand writing data/ba/dev/'
mountain_dev_dir = 'Hand writing data/da/dev/'
opencountry_dev_dir = 'Hand writing data/la/dev/'

n = 20

coast_train = load_dir(coast_train_dir, n)
forest_train = load_dir(forest_train_dir, n)
highway_train = load_dir(highway_train_dir, n)
mountain_train = load_dir(mountain_train_dir, n)
opencountry_train = load_dir(opencountry_train_dir, n)

coast_dev = load_dir(coast_dev_dir, n)
forest_dev = load_dir(forest_dev_dir, n)
highway_dev = load_dir(highway_dev_dir, n)
mountain_dev = load_dir(mountain_dev_dir, n)
opencountry_dev = load_dir(opencountry_dev_dir, n)
...
scores_dev = knn_predict(X_train_pca, y_train, X_dev_pca, classes, k = 15)
dev_pred = np.argmax(scores_dev, axis = -1)
